...
configure_router() {
    local container=$1
    local ip_address=$2
    local multicast_group=$3

    echo "Configuring router with IP $ip_address and multicast group $multicast_group"
    cmd="
        ip address add $ip_address dev eth0 &&
        ip link add name vxlan10 type vxlan id 10 group $multicast_group dstport 4789 dev eth0 &&
    ...
    for container in $(docker ps -q); do
        ...
        case "$container_hostname" in
            $ROUTER1)
                ...
                configure_router $container "20.1.1.1/24" "239.1.1.1"
                ;;
            $ROUTER2)
                echo " - Resetting and Configuring $ROUTER2 -"
                reset_router $container
                configure_router $container "20.1.1.2/24" "239.1.1.1"
                ;;
            $HOST1)
